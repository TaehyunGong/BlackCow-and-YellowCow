...
<div class='all-view'>
   			...
   			<div id='fund-cards' class=" col-sm-3 wow fadeIn">
		           ...
		           <%-- ,<%=list.get(i).getPro_detail_no() %> --%>
		            	<a  style="text-decoration:none; color:black;">
		             	   <img class="img-card" onclick="move(<%=list.get(i).getProduct_no()%>, <%=list.get(i).getRanks()%>);" src='<%=request.getContextPath()%>/images/funding/<%=list.get(i).getOri_fileName()%>'>
		                </a>
  		 ...
  		 function move(num , pro)
  		 {
  			 var pnum = num;
  			 var percent =  pro;
  			 
  			location.href="selectProduct.bo?product_page="+pnum+"&progress="+percent;
  		 }
			 ...
			 if($(btn).attr('style') == undefined){
					<%if(loginUser != null){%>
						...
						$.ajax({
							...
							console.log("ㅅㅄㅄㅂ"+data);
							alert('해당 상품을 삭제하였습니다.');
							
							location.href="<%=request.getContextPath()%>/selectDibs?userNo=" + userNo;
			...
			function dibsOn(){
				...
				<%
					if(list != null){
						for(int i=0; i<list.size(); i++){
				%>
					user_dib += <%=list.get(i).getPro_detail_no() %>+' ';
				<%
						}
					}
				%>
				arr = user_dib.split(' ');
				for(var i in arr){
					$(".far fa-heart'"+arr[i]+"']").css('font-weight','900');
