...
<style>
	...
	#helpAllQnA{display:block; border-bottom:5px solid green;}
   ...
   <%@ include file="../common/navbar.jsp" %>
	...
	<div id="sub-1">
		<ul class="ulsub-1">
			...
			<%-- <li><a class="ul-li-a" href="<%=request.getContextPath() %>/searchAllQna.hp" id="helpAllQnA">문의내역 보기</a></li> --%>
		</ul>
	...
	<div class="pageArea" align="center">
		<button class="active" onclick="location.href='<%=request.getContextPath()%>/searchAllQna.hp?currentPage=1'">≪</button>
		<% if(currentPage <= 1){ %>
			<button class="active" disabled>＜</button>
		<% }else{ %>
			<button onclick="location.href='<%=request.getContextPath()%>/searchAllQna.hp?currentPage=<%=currentPage - 1%>'">＜</button>
		<% } %>
		...
		<% for(int p = startPage; p <= endPage; p++){
			...
			<% }else{ %>
				<button class="active" onclick="location.href='<%=request.getContextPath()%>/searchAllQna.hp?currentPage=<%=p%>'"><%=p %></button>
			<% } %>
		<% } %>
		...
		<% if(currentPage >= maxPage){ %>
			<button class="active" disabled>＞</button>
		<% }else{ %>
			<button class="active" onclick="location.href='<%=request.getContextPath()%>/searchAllQna.hp?currentPage=<%=currentPage + 1%>'">＞</button>
		<% } %>
			<button class="active" onclick="location.href='<%=request.getContextPath()%>/searchAllQna.hp?currentPage=<%=maxPage%>'">≫</button>
	</div>
