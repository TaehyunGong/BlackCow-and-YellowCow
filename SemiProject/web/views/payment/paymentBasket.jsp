...
<%@ page import='com.kh.Semi.basket.model.vo.*,java.util.ArrayList'%>
...
<%
	...
	int totalPrice = 0;								//모든 상품의 토탈 금액
	
	if(request.getAttribute("basketList") != null){	//장바구니에서 가져왔을떄
		basketList = (ArrayList<Basket>)request.getAttribute("basketList");
		addressList = (String[])request.getAttribute("addressList");
		for(Basket b : basketList){
			totalPrice += (b.getPrice()*b.getProduct_count());
		}
	}
%>
<!DOCTYPE html>
   ...
   <div class='all-view'>
   		...
   		<div class='pay-box'>
   			...
   			<%for(int i=0;i<basketList.size();i++){ %>
   				<div class='item'>
   					<div class='item-img' style='background-image: url(<%=request.getContextPath()%>/images/funding/<%=basketList.get(i).getOri_filename()%>);'></div>
   					<div class='item-content'>
   						<label class='item-content-text'>
   							<h4><%=basketList.get(i).getPro_title() %></h4>
   						</label>
   					</div>
   					<div class='item-QTY'>
   						<label class='item-QTY-text'>
   							<label style='font-weight: bold; padding-top: 36px;'>
   								수량 : <%=basketList.get(i).getProduct_count() %>개
   							</label>
   						</label>
   					</div>
   					<div class='item-price'>
   						<label style='font-weight: bold; padding-top: 36px;'>
   								<%=basketList.get(i).getPrice() * basketList.get(i).getProduct_count() %>원
   							</label><br>
   							<label style='color:gray; font-size: 12px;'>
   								한 개당:<%=basketList.get(i).getPrice() %>원
   						</label>
   					</div>
   				</div>
   			<%} %>
   				...
   				<div class='result-sum'>
   					<table class='result-table'>
   						...
   						<tr>
   							...
   							<td style='color: blue;font-size: 30px;font-weight: bold;'><%=totalPrice + 2500 %>원</td>
   						</tr>
   					</table>
...
<script>
  ...
  function cash(){
      ...
      IMP.request_pay({
          ...
          name : '<%=basketList.get(0).getPro_title()%>등',
          amount : <%=totalPrice%>,		//여기
  ...
  for(Basket b : basketList){
	...
	gs = gson.toJson(basketList);
  %>
  function throwPayment(imp, addr){
	  var $form = $('<form></form>');
	     $form.attr('action', '<%=request.getContextPath()%>/PaymentViewBasket');
	     $form.attr('method', 'post');
	     $form.appendTo('body');
	     
	     var idx = $('<input type="hidden" value='+imp+' name="imp">');
	     var page = $('<input type="hidden" value=<%=gs%> name="basketList">');
	     var addr = $('<input type="hidden" value='+addr+' name="address">');
	     $form.append(idx).append(page).append(addr);
	     $form.submit();
  }
</script>
