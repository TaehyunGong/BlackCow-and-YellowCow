                  ...
                  <table id="datatable" class="table table-striped table-bordered">
                    ...
                    <% for(Member m : list) { %>
                    	<tr>
                    		...
                    		<% String type = "";
                    			switch(m.getMember_Type()){ 
                    			case "M1" : type = "간단회원"; break;
                    			case "M2" : type = "일반회원"; break;
                    		} %>
                    		<td><%=type %></td>
                    		<td class="farm_name"><%=m.getFarm_Name() %></td>
                    		<td class="farm_address"><%=m.getFarm_Address() %></td>
                    		<td class="farm_size"><%=m.getFarm_Size() %></td>
                    		<td>
                    			<button class="btn btn-success btn-xs">승인</button>
                    			<button class="btn btn-danger btn-xs">거절</button>
                    		</td>
                    	</tr>
                    <% } %>
                    </tbody>
		...
		<script>
			$(function(){
				$('td button:nth-child(1)').click(function(){
					var memberNo = $(this).parent().parent().children('input').val();
					$.ajax({
						url : "<%=request.getContextPath()%>/goFarmer",
						data : {memberNo:memberNo},
						success : function(data){
							var result = data.result;
							if(result > 0){
								alert('선택하신 회원의 농가 신청이 승인되었습니다.');
								location.href="<%=request.getContextPath()%>/farmApprove";
							}else{
								console.log('농가승인 실패');
							}
						}
				...
				$('td button:nth-child(2)').click(function(){
					var memberNo = $(this).parent().parent().children('input').val();
					$.ajax({
						url : "<%=request.getContextPath()%>/rejectFarmer",
						data : {memberNo:memberNo},
						success : function(data){
							var result = data;
							if(result > 0){
								alert('선택하신 회원의 농가 신청이 거절되었습니다.');
								location.href="<%=request.getContextPath()%>/farmApprove";
							}else{
								console.log('농가거절 실패');
							}
