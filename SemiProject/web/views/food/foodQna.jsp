...
font-family: 'Nanum Pen Script', cursive; font-size:80px;"><%=intro.get("pro_name") %></p>
	...
	<div id="tabMenu">
		...
		<button class="tablinks active" onclick="category(event)" value="btn5" id="mun">문의   </button>
	</div>
		...
		<div class="leftArea">
			<div class="head">
				...
				<div class="replyCount">
				 	<span><%=list.size() %>개</span>의 문의가 있습니다
				</div>
			</div>
			...
			<div class="replyArea">
				...
				<div class="btnsubmit">
					<% if(loginUser != null) {%>
					<button type="submit" id="checkReplyContent">문의하기</button>
					<script>
					...
					<% }else{ %>
					<button type="button" onclick="noMemberUser()">문의하기</button>
					<% } %>
				</div>
			</div>
			<% if(loginUser != null) { %>
			...
			<input type="hidden" name="refBoard" value="R5">
