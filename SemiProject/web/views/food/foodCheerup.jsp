...
font-family: 'Nanum Pen Script', cursive; font-size:80px;"><%=intro.get("pro_name") %></p>
	...
	<div id="tabMenu">
		...
		<button class="tablinks active" onclick="category(event)" id="cheerUp"
		...
		<div class="leftArea">
			<div class="head">
				...
				<div class="replyCount">
				 	<span><%=list.size() %>개</span>의 응원메세지가 있습니다
				</div>
			</div>
			...
			<div class="replyArea">
				...
				<div class="btnsubmit">
					<% if(loginUser != null) {%>
					<button type="submit" id="checkReplyContent">응원하기</button>
					<script>
			...
			<input type="hidden" name="refBoard" value="R3">
			...
			<div class="reListArea">
				...
				<div class="reContent">
					...
					<div class="replysubmit">
						...
						<%if(loginUser != null){ %>
						<button onclick="deleteboard(<%=list.get(i).getBoard_No() %>,<%=list.get(i).getMember_No() %>);" value="<%=list.get(i).getBoard_No() %>">삭제</button>
						<% } %>
					</div>
				</div>
				<hr>
...
//	            console.log(d)
	...
	//게시댓글 삭제
	function deleteboard(bnum,member_no){
		
		var bnum = bnum; // 보드번호
		var mnum = member_no; // 작성자 번호
		var proDnum = <%=pro_Detail_No%>; // 상품번호
		var type = "B5";
		var board = "R3";
		
		var loginNo = <%=loginUser.getMember_No()%>; // 로그인유저번호
		
		
		<%if(loginUser.getLogin_Type().equals("M4") 
				|| loginUser.getMember_Id().equals(intro.get("member_Id")) ) {%>
